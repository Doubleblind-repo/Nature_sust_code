...
base_run_path = base_path / '01 Decentralised' #base run Paths
...
for db in [db_base.get(k) for k in ['NRW_NoIns','VIC_NoIns']]:
    ...
    if i == 0:
        ...
        draw_text(ax, '(a) - NRW', SMALL_SIZE, 'upper left')
        ...
        ax.set( yticklabels = [], 
            xlabel="$Q/L \: [\mathrm{MWh} \: \mathrm{m}^{-1}]$")        
        draw_text(ax,'(b) - VIC', SMALL_SIZE, 'upper left')
        draw_text(ax, 'Decentralised', SMALL_SIZE, 'lower right')
...
for i,fig in enumerate(fig_list):
    fig.savefig(f'Fig_{i}_Decen.pdf', dpi=1000)
